...
struct intBundle
{
  ...
  arma::mat oldT1,oldT2aa,oldT2ab;
...
int main(int argc, char *argv[])
{
  ...
  std::string pointGroup,basis;
  int method;
  char xyzName[300];
  char punchName[300];

  readInputFile(argv[1],xyzName,punchName,basis,pointGroup,method);


  std::ifstream input_file(xyzName);
  ...
  std::vector<int> nAOs;
  std::vector<std::vector<int> > modes;
  applySym2(atomsUnique,basis,pointGroup,atoms,nAOs,ao2mo,bas,prodTable,modes);
  std::vector<arma::mat> Ss,K1s,Cs;
  std::vector<std::vector<arma::mat> > V2s;
  std::vector<int> nOcc,nCore;
...
arma::mat T1,T2aa,T2ab;
  ...
  {
    if(atoms[a].atomic_number > 2)
    {
      core += 1;
    }
    if(atoms[a].atomic_number > 10)
    {
      core += 4;
    }
    if(atoms[a].atomic_number > 18)
    {
      core += 4;
    }
    if(atoms[a].atomic_number > 36)
    {
      core += 9;
    }
    if(atoms[a].atomic_number > 54)
    {
      core += 9;
    }
    if(atoms[a].atomic_number > 86)
    {
      core += 16;
    }
  }
  ...
  /*runPara(atoms,basis,nAOs,ao2mo,bas,prodTable,Ep,T1,T2aa,T2ab,K1s,V2s,Cs,nOcc,nCore,true,true,core);
  //runPara(atoms,basis,nAOs,ao2mo,bas,prodTable,Ep,T1,T2aa,T2ab,K1s,V2s,Cs,nOcc,nCore,true,true,core);
  atoms[0].x += 0.01;
  std::vector<arma::mat> Cs2=Cs;
  runPara(atoms,basis,nAOs,ao2mo,bas,prodTable,Ep,T1,T2aa,T2ab,K1s,V2s,Cs,nOcc,nCore,true,true,core);
  //runPara(atoms,basis,nAOs,ao2mo,bas,prodTable,Ep,T1,T2aa,T2ab,K1s,V2s,Cs,nOcc,nCore,true,true,core);
  Cs2[0].print();
  std::cout << std::endl;
...
return 0;
  ...
  Cs.resize(nAOs.size());
  double Ehf=solveHFsym(nels,Cs,Ss,K1s,V2s,bas,nOcc,nCore,nAOs,true,core);
  std::cout << Enuc+Ehf << std::endl;

  atoms[0].x += 0.00001;
  genInts(Enuc,atoms,basis,Ss,K1s,V2s,ao2mo,bas,nAOs,nels,prodTable);

  Ehf=solveHFsym(nels,Cs,Ss,K1s,V2s,bas,nOcc,nCore,nAOs,true,core);
  std::cout << Enuc+Ehf << std::endl;

  return 0;
*/

  bundleNuc integrals={atomsUnique,basis,pointGroup,nAOs,ao2mo,bas,prodTable,modes,core,0,0.0};
  integrals.ts.resize(100);
  integrals.tLabel.resize(100);
  integrals.g.resize(modes.size());
  integrals.oldE=0.0;
  integrals.numE=integrals.numPe=0;
  integrals.punchName=punchName;

  setTime(t2,t2,time_span);

  for(int i=0;i<100;i++)
  {
    integrals.ts[i]=time_span;
    integrals.tLabel[i]="";
  }

  double* Ts;
  int numTot=modes.size();
  ...
  for(int m=0;m<modes.size();m++)
  {
    ...
    {
      case 0:
	Ts[m] = atomsUnique[modes[m][0]].x;
	break;
      case 1:
	Ts[m] = atomsUnique[modes[m][0]].y;
	break;
      case 2:
	Ts[m] = atomsUnique[modes[m][0]].z;
	break;
    }
  }
  ...
  double E;
  //gradEvalU(T,E,G,&integrals);
  //gradEvalZ(T,E,G,&integrals);
  //numEval(T,E,G,&integrals);
  //return 0;
  ...
  minlbfgscreate(std::min(100,numTot),T,state);
...
//  std::cout << "Iter\tE Corr\t\tdE\t\t\tlargest dE/dT" << std::endl;
  ...
  {
    minlbfgsoptimize(state,gradEvalU,printItsNuc,&integrals);
  }
  else if(method == 2)
  {
    minlbfgsoptimize(state,gradEvalZ,printItsNuc,&integrals);
  }
  //minlbfgsresults(state,T,rep);

  for(int i=0;i<integrals.ts.size();i++)
  {
    if(integrals.tLabel[i] != "")
    {
      std::cout << integrals.tLabel[i] << ":\t";
      std::cout << integrals.ts[i].count() << std::endl;
    }
  }
...
//std::cout << std::endl;
    //std::cout << "Iter\tE Corr\t\tdE\t\t\tlargest dE/dT" << std::endl;
    //printf("%d\t%.7f\t%.4E\t%.4E\t%.4E\t%.4E\t%.4E\t%.4E\n",(*iPtr).numIt+1,(*iPtr).E,(*iPtr).E-(*iPtr).oldE,(*iPtr).maxT,iPtr->stuff[0],iPtr->stuff[1],iPtr->stuff[2],iPtr->stuff[3]);
    printf("%d\t%.9f\t%.10E\t%.10E",(*iPtr).numIt+1,(*iPtr).E,(*iPtr).E-(*iPtr).oldE,(*iPtr).maxT);
    std::cout << std::endl;
  }
  (*iPtr).numIt++;
  (*iPtr).oldE=(*iPtr).E;
...
//std::cout << "hello" << std::endl;
    iPtr->oldT1=iPtr->T1;
    iPtr->oldT2aa=iPtr->T2aa;
    iPtr->oldT2ab=iPtr->T2ab;
    iPtr->T1=T1;
    iPtr->T2aa=T2aa;
    iPtr->T2ab=T2ab;
  }
...
/*  double minF=0.00001;
  //double minF=1.0;
  double sqF=sqrt(minF);
  //x=.997688;

  arma::mat blue1m(F1m.n_rows,F1m.n_cols);
  for(int e=0;e<virt;e++)
  {
    for(int m=0;m<nels;m++)
    {
      if(F1m(m,e) < minF)
      {
	blue1m(m,e)=0.0;//-sqF*exp(-0.5*F1m(m,e)/minF+0.5)+2.0*sqF;
	//blue1m(m,e)=-sqF*exp(-0.5*F1m(m,e)/minF+0.5)+2.0*sqF;
      }
      else
      {
	//blue1m(m,e)=x;
	blue1m(m,e)=0.0;//sqrt(F1m(m,e));
	//blue1m(m,e)=sqrt(F1m(m,e));
      }
    }
  }
  arma::mat red1m(F1m.n_rows,F1m.n_cols);
  for(int e=0;e<virt;e++)
  {
    for(int m=0;m<nels;m++)
    {
      if(F1m(m,e) < minF)
      {
	red1m(m,e)=0.0;//1.0/sqF*exp(-0.5*F1m(m,e)/minF+0.5);
	//red1m(m,e)=1.0/sqF*exp(-0.5*F1m(m,e)/minF+0.5);
      }
      else
      {
	red1m(m,e)=0.0;//1.0/sqrt(F1m(m,e));
	//red1m(m,e)=1.0/sqrt(F1m(m,e));
      }
    }
  }
  arma::mat blue2aaM(F2aaM.n_rows,F2aaM.n_cols);
  for(int e=0;e<virt;e++)
  {
    for(int f=e+1;f<virt;f++)
    {
	...
	for(int n=m+1;n<nels;n++)
	{
	  if(fabs(iPtr->V2aaij_ab(indexD(m,n,nels),indexD(e,f,virt)))<0.0000000001)
	  {
	    blue2aaM(indexD(m,n,nels),indexD(e,f,virt))=1.0;//-sqF*exp(-0.5*F2aaM(indexD(m,n,nels),indexD(e,f,virt))/minF+0.5)+2.0*sqF;
	  }
	  else if(F2aaM(indexD(m,n,nels),indexD(e,f,virt)) < minF)
	  {
	    blue2aaM(indexD(m,n,nels),indexD(e,f,virt))=0.0;//-sqF*exp(-0.5*F2aaM(indexD(m,n,nels),indexD(e,f,virt))/minF+0.5)+2.0*sqF;
	    //blue2aaM(indexD(m,n,nels),indexD(e,f,virt))=-sqF*exp(-0.5*F2aaM(indexD(m,n,nels),indexD(e,f,virt))/minF+0.5)+2.0*sqF;
	  }
	  else
	  {
	    //blue2aaM(indexD(m,n,nels),indexD(e,f,virt))=x;
	    blue2aaM(indexD(m,n,nels),indexD(e,f,virt))=sqrt(F2aaM(indexD(m,n,nels),indexD(e,f,virt)));
	  }
	}
      }
    }
  }
  arma::mat red2aaM(F2aaM.n_rows,F2aaM.n_cols);
  for(int e=0;e<virt;e++)
  {
    for(int f=e+1;f<virt;f++)
    {
	...
	for(int n=m+1;n<nels;n++)
	{
	  if(fabs(iPtr->V2aaij_ab(indexD(m,n,nels),indexD(e,f,virt)))<0.0000000001)
	  {
	    red2aaM(indexD(m,n,nels),indexD(e,f,virt))=0.0;//-sqF*exp(-0.5*F2aaM(indexD(m,n,nels),indexD(e,f,virt))/minF+0.5)+2.0*sqF;
	  }
	  else if(F2aaM(indexD(m,n,nels),indexD(e,f,virt)) < minF)
	  {
	    red2aaM(indexD(m,n,nels),indexD(e,f,virt))=0.0;//1.0/sqF*exp(-0.5*F2aaM(indexD(m,n,nels),indexD(e,f,virt))/minF+0.5);
	    //red2aaM(indexD(m,n,nels),indexD(e,f,virt))=1.0/sqF*exp(-0.5*F2aaM(indexD(m,n,nels),indexD(e,f,virt))/minF+0.5);
	  }
	  else
	  {
	    red2aaM(indexD(m,n,nels),indexD(e,f,virt))=1.0/sqrt(F2aaM(indexD(m,n,nels),indexD(e,f,virt)));
	  }
	}
      }
    }
  }
  arma::mat blue2abM(F2abM.n_rows,F2abM.n_cols);
  for(int e=0;e<virt;e++)
  {
    for(int f=0;f<virt;f++)
    {
	...
	for(int n=0;n<nels;n++)
	{
	  if(fabs(iPtr->V2abij_ab(m*nels+n,e*virt+f))<0.0000000001)
	  {
	    blue2abM(m*nels+n,e*virt+f)=1.0;//-sqF*exp(-0.5*F2abM(m*nels+n,e*virt+f)/minF+0.5)+2.0*sqF;
	  }
	  else if(F2abM(m*nels+n,e*virt+f) < minF)
	  {
	    blue2abM(m*nels+n,e*virt+f)=0.0;//-sqF*exp(-0.5*F2abM(m*nels+n,e*virt+f)/minF+0.5)+2.0*sqF;
	    //blue2abM(m*nels+n,e*virt+f)=-sqF*exp(-0.5*F2abM(m*nels+n,e*virt+f)/minF+0.5)+2.0*sqF;
	  }
	  else
	  {
	    //blue2abM(m*nels+n,e*virt+f)=x;
	    blue2abM(m*nels+n,e*virt+f)=sqrt(F2abM(m*nels+n,e*virt+f));
	  }
	}
      }
    }
  }  
  arma::mat red2abM(F2abM.n_rows,F2abM.n_cols);
  for(int e=0;e<virt;e++)
  {
    for(int f=0;f<virt;f++)
    {
	...
	for(int n=0;n<nels;n++)
	{
	  if(fabs(iPtr->V2abij_ab(m*nels+n,e*virt+f))<0.0000000001)
	  {
	    red2abM(m*nels+n,e*virt+f)=0.0;//-sqF*exp(-0.5*F2abM(m*nels+n,e*virt+f)/minF+0.5)+2.0*sqF;
	  }
	  else if(F2abM(m*nels+n,e*virt+f) < minF)
	  {
	    red2abM(m*nels+n,e*virt+f)=0.0;//1.0/sqF*exp(-0.5*F2abM(m*nels+n,e*virt+f)/minF+0.5);
	    //red2abM(m*nels+n,e*virt+f)=1.0/sqF*exp(-0.5*F2abM(m*nels+n,e*virt+f)/minF+0.5);
	  }
	  else
	  {
	    red2abM(m*nels+n,e*virt+f)=1.0/sqrt(F2abM(m*nels+n,e*virt+f));
	  }
	}
      }
    }
...
//std::cout << "\t\t\t" << iPtr->E << std::endl;

  //(*iPtr).E += 4.0*trace((*iPtr).K1ia*arma::vectorise(T1%blue1m));
...
//std::cout << "\t\t\t" << iPtr->E << std::endl;
  ...
  //(*iPtr).E += 2.0*accu((*iPtr).V2abij_ab%T2ab%blue2abM);
//std::cout << "\t\t\t" << iPtr->E << std::endl;
  //(*iPtr).E += 2.0*accu((*iPtr).V2aaij_ab%T2aa%blue2aaM);
//std::cout << "\t\t\t" << iPtr->E << std::endl;
  ...
  for(int e=0;e<virt;e++)
  {
    for(int m=0;m<nels;m++)
    {
      //P1(m,e)=4.0*(*iPtr).K1ia(e*nels+m)*T1(m,e)*red1m(m,e);
  ...
  for(int e=0;e<virt;e++)
  {
	...
	for(int n=0;n<nels;n++)
	{
	  //P2ab(m*nels+n,n1) = 2.0*(*iPtr).V2abij_ab(m*nels+n,e*virt+f)*T2ab(m*nels+n,n1)*red2abM(m*nels+n,n1);
  ...
  for(int e=0;e<virt;e++)
  {
	...
	for(int n=m+1;n<nels;n++)
	{
	  n3=indexD(m,n,nels);
	  n4=indexD(m,n,nact);
	  //P2aa(n3,n1) = 2.0*(*iPtr).V2aaij_ab(n3,n1)*T2aa(n3,n1)*red2aaM(n3,n1);
  ...
  for(int a=0;a<virt;a++)
  {
    for(int i=0;i<nels;i++)
    {
      //x=4.0*(*iPtr).K1ia(a*nels+i)*blue1m(i,a);
  ...
  for(int a=0;a<virt;a++)
  {
        ...
        for(int j=i+1;j<nels;j++)
	{
	  ...
	  //x+= 2.0*(*iPtr).V2aaij_ab(indexD(i,j,nels),indexD(a,b,virt))*blue2aaM(n5+j,n3);
  ...
  for(int a=0;a<virt;a++)
  {
    for(int i=0;i<nels;i++)
    {
      ...
      //y += 2.0*(*iPtr).V2abij_ab(i*nels+i,a*virt+a)*blue2abM(i*nels+i,a*virt+a);
  ...
  for(int a=0;a<virt;a++)
  {
	...
	for(int j=0;j<nels && (b*nels+j)<a*nels+i;j++)
	{
	  y=2.0*((*iPtr).K1ia(a*nels+i)*T1(j,b)+(*iPtr).K1ia(b*nels+j)*T1(i,a));
          //y += 2.0*(*iPtr).V2abij_ab(i*nels+j,a*virt+b)*blue2abM(i*nels+j,a*virt+b);
...
//  std::cout << std::endl;
  ...
  for(int e=0;e<virt;e++)
  {
    ...
    {
      x += T1(m,e)*T1(m,e);
      if(fabs(T1(m,e)) > fabs(stuff[0]))
      {
	stuff[0]=T1(m,e);
	stuff[1]=grad[e*nels+m];
      }
      if(F1m(m,e) < stuff[3])
      {
	stuff[3]=F1m(m,e);
  ...
  for(int e=0;e<virt;e++)
  {
	...
	for(int n=m+1;n<nels;n++)
	{
	  x += T2aa(indexD(m,n,nels),indexD(e,f,virt))*T2aa(indexD(m,n,nels),indexD(e,f,virt));
	  if(fabs(T2aa(indexD(m,n,nels),indexD(e,f,virt))) > fabs(stuff[0]))
	  {
	    stuff[0]=T2aa(indexD(m,n,nels),indexD(e,f,virt));
	    stuff[1]=grad[n1a+indexD(e,f,virt)*T2aa.n_rows+indexD(m,n,nels)];
	  }
	  if(F2aaM(indexD(m,n,nels),indexD(e,f,virt)) < stuff[3])
	  {
	    stuff[3]=F2aaM(indexD(m,n,nels),indexD(e,f,virt));
	  }
	}
      }
    }
  ...
  for(int e=0;e<virt;e++)
  {
	...
	for(int n=0;n<nels;n++)
	{
	  x += T2ab(m*nels+n,e*virt+f)*T2ab(m*nels+n,e*virt+f);
	  if(fabs(T2ab(m*nels+n,e*virt+f)) > fabs(stuff[0]))
	  {
	    stuff[0]=T2ab(m*nels+n,e*virt+f);
	    stuff[1]=grad[n1a+n2aa+(e*virt+f)*T2ab.n_rows+m*nels+n];
	  }
	  if(F2abM(m*nels+n,e*virt+f) < stuff[3])
	  {
	    stuff[3]=F2abM(m*nels+n,e*virt+f);
	  }
	}
      }
    }
  ...
  int numPrint=10;
  std::vector<double> maxT(numPrint,0.0);
  std::vector<double> maxTaa(numPrint,0.0);
  std::vector<double> maxT1a(numPrint,0.0);
  std::vector<std::vector<int> > maxInd(numPrint,std::vector<int>(4));
  std::vector<std::vector<int> > maxIndAA(numPrint,std::vector<int>(4));
  std::vector<std::vector<int> > maxInd1(numPrint,std::vector<int>(2));

  std::vector<double> minFab(numPrint,1.0);
  std::vector<double> minFaa(numPrint,1.0);
  std::vector<double> minF1(numPrint,1.0);
  std::vector<std::vector<int> > minIndF(numPrint,std::vector<int>(4));
  std::vector<std::vector<int> > minIndFaa(numPrint,std::vector<int>(4));
  std::vector<std::vector<int> > minIndF1(numPrint,std::vector<int>(2));
  

  int num;
  std::vector<int> tmpI(4);
  std::vector<int> tmpI1(4);
  double tmp;
  for(int e=0;e<virt;e++)
  {
    for(int m=0;m<nels;m++)
    {
      if(fabs(T1(m,e)) > fabs(maxT1a[0]))
      {
	maxT1a[0]=T1(m,e);
	maxInd1[0][0]=m; maxInd1[0][1]=e;
	num=0;
	while(num<numPrint-1 && fabs(maxT1a[num])>fabs(maxT1a[num+1]))
	{
	  tmp=maxT1a[num+1];
	  tmpI1=maxInd1[num+1];
	  maxT1a[num+1]=maxT1a[num];
	  maxInd1[num+1]=maxInd1[num];
	  maxT1a[num]=tmp;
	  maxInd1[num]=tmpI1;
	  num++;
	}
  ...
  for(int e=0;e<virt;e++)
  {
    for(int m=0;m<nels;m++)
    {
      ...
      {
	minF1[0]=F1m(m,e);
	minIndF1[0][0]=m; minIndF1[0][1]=e;
	num=0;
	while(num<numPrint-1 && minF1[num]<minF1[num+1])
	{
	  tmp=minF1[num+1];
	  tmpI1=minIndF1[num+1];
	  minF1[num+1]=minF1[num];
	  minIndF1[num+1]=minIndF1[num];
	  minF1[num]=tmp;
	  minIndF1[num]=tmpI1;
	  num++;
	}
  ...
  for(int e=0;e<virt;e++)
  {
	...
	for(int n=m+1;n<nels;n++)
	{
	  if(fabs(T2aa(indexD(m,n,nels),indexD(e,f,virt))) >= fabs(maxTaa[0]))
	  {
	    maxTaa[0]=T2aa(indexD(m,n,nels),indexD(e,f,virt));
	    maxIndAA[0][0]=m; maxIndAA[0][1]=n;
	    maxIndAA[0][2]=e; maxIndAA[0][3]=f;
	    num=0;
	    while(num<numPrint-1 && fabs(maxTaa[num])>=fabs(maxTaa[num+1]))
	    {
	      tmp=maxTaa[num+1];
	      tmpI=maxIndAA[num+1];
	      maxTaa[num+1]=maxTaa[num];
	      maxIndAA[num+1]=maxIndAA[num];
	      maxTaa[num]=tmp;
	      maxIndAA[num]=tmpI;
	      num++;
	    }
  ...
  for(int e=0;e<virt;e++)
  {
	...
	for(int n=m+1;n<nels;n++)
	{
	  if(F2aaM(indexD(m,n,nels),indexD(e,f,virt)) <= minFaa[0])
	  {
	    minFaa[0]=F2aaM(indexD(m,n,nels),indexD(e,f,virt));
	    minIndFaa[0][0]=m; minIndFaa[0][1]=n;
	    minIndFaa[0][2]=e; minIndFaa[0][3]=f;
	    num=0;
	    while(num<numPrint-1 && minFaa[num]<=minFaa[num+1])
	    {
	      tmp=minFaa[num+1];
	      tmpI=minIndFaa[num+1];
	      minFaa[num+1]=minFaa[num];
	      minIndFaa[num+1]=minIndFaa[num];
	      minFaa[num]=tmp;
	      minIndFaa[num]=tmpI;
	      num++;
	    }
  ...
  for(int e=0;e<virt;e++)
  {
	...
	for(int n=0;n<nels;n++)
	{
	  if(fabs(T2ab(m*nels+n,e*virt+f)) > fabs(maxT[0]))
	  {
	    maxT[0]=T2ab(m*nels+n,e*virt+f);
	    maxInd[0][0]=m; maxInd[0][1]=n;
	    maxInd[0][2]=e; maxInd[0][3]=f;
	    num=0;
	    while(num<numPrint-1 && fabs(maxT[num])>fabs(maxT[num+1]))
	    {
	      tmp=maxT[num+1];
	      tmpI=maxInd[num+1];
	      maxT[num+1]=maxT[num];
	      maxInd[num+1]=maxInd[num];
	      maxT[num]=tmp;
	      maxInd[num]=tmpI;
	      num++;
	    }
  ...
  for(int e=0;e<virt;e++)
  {
	...
	for(int n=0;n<nels;n++)
	{
	  if(F2abM(m*nels+n,e*virt+f) < minFab[0])
	  {
	    minFab[0]=F2abM(m*nels+n,e*virt+f);
	    minIndF[0][0]=m; minIndF[0][1]=n;
	    minIndF[0][2]=e; minIndF[0][3]=f;
	    num=0;
	    while(num<numPrint-1 && minFab[num]<minFab[num+1])
	    {
	      tmp=minFab[num+1];
	      tmpI=minIndF[num+1];
	      minFab[num+1]=minFab[num];
	      minIndF[num+1]=minIndF[num];
	      minFab[num]=tmp;
	      minIndF[num]=tmpI;
	      num++;
	    }
  ...
  printf("\t\t%.5E\t%.5E\t%.5E\t%.5E\t%.5E\n",E,stuff[0],stuff[1],stuff[2],stuff[3]);
  std::cout << std::endl;
  for(int i=0;i<numPrint;i++)
  {
    x=grad[n1a+n2aa+(maxInd[i][2]*virt+maxInd[i][3])*T2ab.n_rows+maxInd[i][0]*nels+maxInd[i][1]];
    y=iPtr->V2abij_ab(maxInd[i][0]*nels+maxInd[i][1],maxInd[i][2]*virt+maxInd[i][3]);
    printf("\t\t%d %d %d %d\t%.5E\t%.5E\t%.5E\n",maxInd[i][0],maxInd[i][1],maxInd[i][2],maxInd[i][3],maxT[i],x,y);
  }
  std::cout << std::endl;
  for(int i=0;i<numPrint;i++)
  {
    x=grad[n1a+indexD(maxIndAA[i][2],maxIndAA[i][3],virt)*T2aa.n_rows+indexD(maxIndAA[i][0],maxIndAA[i][1],nels)];
    y=iPtr->V2aaij_ab(indexD(maxIndAA[i][0],maxIndAA[i][1],nels),indexD(maxIndAA[i][2],maxIndAA[i][3],virt));
    printf("\t\t%d %d %d %d\t%.5E\t%.5E\t%.5E\n",maxIndAA[i][0],maxIndAA[i][1],maxIndAA[i][2],maxIndAA[i][3],maxTaa[i],x,y);
  }
  std::cout << std::endl;
  for(int i=0;i<numPrint;i++)
  {
    x=grad[maxInd1[i][1]*nels+maxInd1[i][0]];
    printf("\t\t%d %d\t%.5E\t%.5E\n",maxInd1[i][0],maxInd1[i][1],maxT1a[i],x);
  }
  std::cout << std::endl;
  for(int i=0;i<numPrint;i++)
  {
    x=grad[n1a+n2aa+(minIndF[i][2]*virt+minIndF[i][3])*T2ab.n_rows+minIndF[i][0]*nels+minIndF[i][1]];
    y=iPtr->V2abij_ab(minIndF[i][0]*nels+minIndF[i][1],minIndF[i][2]*virt+minIndF[i][3]);
    printf("\t\t%d %d %d %d\t%.5E\t%.5E\t%.5E\n",minIndF[i][0],minIndF[i][1],minIndF[i][2],minIndF[i][3],minFab[i],x,y);
  }
  std::cout << std::endl;
  for(int i=0;i<numPrint;i++)
  {
    x=grad[n1a+indexD(minIndFaa[i][2],minIndFaa[i][3],virt)*T2aa.n_rows+indexD(minIndFaa[i][0],minIndFaa[i][1],nels)];
    y=iPtr->V2aaij_ab(indexD(minIndFaa[i][0],minIndFaa[i][1],nels),indexD(minIndFaa[i][2],minIndFaa[i][3],virt));
    printf("\t\t%d %d %d %d\t%.5E\t%.5E\t%.5E\n",minIndFaa[i][0],minIndFaa[i][1],minIndFaa[i][2],minIndFaa[i][3],minFaa[i],x,y);
  }
  std::cout << std::endl;
  for(int i=0;i<numPrint;i++)
  {
    printf("\t\t%d %d\t%.5E\n",minIndF1[i][0],minIndF1[i][1],minF1[i]);
  }
  std::cout << std::endl;
...
/*  double rat=0.0;
  //int n1a=nels*virt;
  //int n2aa=integrals.T2aa.n_rows*integrals.T2aa.n_cols;
fail=false;
  if(fail)
  {
    std::cout << std::endl;
    std::cout << integrals.oldT2ab.max() << '\t' << integrals.oldT2ab.min() << std::endl;
    for(int i=0;i<21;i++)
    {
      //rat=0.0;
      rat=0.05*i;
    for(int e=0;e<virt;e++)
    {
      for(int m=0;m<nels;m++)
      {
	//Ts[e*nels+m]=integrals.oldT1(m,e);//+rat*integrals.T1(m,e);
	Ts[e*nels+m]=(1.0-rat)*integrals.oldT1(m,e)+rat*integrals.T1(m,e);
      }
    }
    for(int a=0;a<virt;a++)
    {
	...
	for(int i=0;i<nels;i++)
	{
	  for(int j=i+1;j<nels;j++)
	  {
	    //Ts[n1a+indexD(a,b,virt)*integrals.T2aa.n_rows+indexD(i,j,nels)]=integrals.oldT2aa(indexD(i,j,nels),indexD(a,b,virt));//+rat*integrals.T2aa(indexD(i,j,nels),indexD(a,b,virt));
	    Ts[n1a+indexD(a,b,virt)*integrals.T2aa.n_rows+indexD(i,j,nels)]=(1.0-rat)*integrals.oldT2aa(indexD(i,j,nels),indexD(a,b,virt))+rat*integrals.T2aa(indexD(i,j,nels),indexD(a,b,virt));
	  }
	}
      }
    }
	...
	for(int i=0;i<nels;i++)
	{
	  for(int j=0;j<nels;j++)
	  {
	    //Ts[n1a+n2aa+(a*virt+b)*integrals.T2ab.n_rows+(i*nels+j)]=integrals.oldT2ab(i*nels+j,a*virt+b);//+rat*integrals.T2ab(i*nels+j,a*virt+b);
	    Ts[n1a+n2aa+(a*virt+b)*integrals.T2ab.n_rows+(i*nels+j)]=(1.0-rat)*integrals.oldT2ab(i*nels+j,a*virt+b)+rat*integrals.T2ab(i*nels+j,a*virt+b);
	  }
	}
      }
    }
    T.setcontent(numTot,Ts);
  real_1d_array grad=T;
      evalE(T,E,grad,&integrals);
    }
...
void gradEvalZ(const real_1d_array &R,double &E,real_1d_array &grad,void *ptr)
{
  ...
  makeD2(bPtr->T1,bPtr->T2aa,bPtr->T2ab,bPtr->D1,bPtr->D2aa,bPtr->D2ab);

  arma::vec eigval;
  arma::mat eigvec;
  eig_sym(eigval,eigvec,bPtr->D1);
  punchFile.open(bPtr->punchName,std::fstream::app);
  char cstr[100];
  std::string str;
  ...
  std::vector<arma::mat> D1s;
  std::vector<std::vector<arma::mat> > D2aaS,D2abS;
  rotateForward(bPtr->nOcc,bPtr->D1,bPtr->D2aa,bPtr->D2ab,nMO,D1s,D2aaS,D2abS,(*bPtr).bas,nCore);
